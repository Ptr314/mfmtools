...
unsigned char * lzss_unpack(unsigned char *packeddata,unsigned int size, unsigned char header_size, unsigned int * new_size )
{
    ...
    buffer_size=size;
    buffer_offset=0;
    buffer_ptr=packeddata+header_size;
    ...
    finalbuffer=0;
    finalbuffer=malloc(j+header_size);
    if(finalbuffer)
    {
        memcpy(finalbuffer,packeddata,header_size);
        memcpy(&finalbuffer[header_size],buffer,j);

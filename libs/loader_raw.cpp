...
uint8_t LoaderRAW::load(bool check_only, bool ignore_warnings, QString *log)
{
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        log->append(tr("File open error\r\n"));
        return FDD_LOAD_ERROR;
    } else {
        log->append(QString(tr("Actual file size: %1\r\n")).arg(QString::number(file.size())));
        log->append(QString(tr("Expected file size: %1\r\n")).arg(QString::number(this->image_size)));
        if (file.size() < this->image_size) {
            log->append(tr("Warning: File size is too small.\r\n"));
            res = FDD_LOAD_SIZE_SMALLER;
        } else
        if (file.size() > this->image_size) {
            log->append(tr("Warning: File size is too large.\r\n"));
            res = FDD_LOAD_SIZE_LARGER;
        } else res = FDD_LOAD_OK;
    }
    if (file.size() != this->image_size) {
        log->append(tr("Trying to calculate variants\r\n"));
        int found = 0;
    ...
    if (res==FDD_LOAD_OK) {
        log->append(tr("No problems found\r\n"));
    }

...
WriterRAW::WriterRAW(Loader *loader)
    :Writer(loader)
{

}

QString WriterRAW::get_file_ext(){
    return QString(".img");
}

uint8_t WriterRAW::write(QString FileName)
{
    QFile file(FileName);
    ...
    for (uint8_t track = 0; track < this->loader->fdd_format["tracks"].toInt(); track++){
        for (uint8_t head = 0; head < this->loader->fdd_format["heads"].toInt(); head++){
            for (uint8_t sector = 1; sector <= this->loader->fdd_format["sectors"].toInt(); sector++){
                ...
                file.write((char*)data, this->loader->fdd_format["sector"].toInt());
            }
        }
    }
